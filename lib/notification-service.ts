...
class NotificationService {
      ...
      body: (data) => `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
          ...
          <div style="margin-bottom: 20px;">
            ...
            <p>${data.shippingAddress?.name || data.customerName}</p>
            <p>${data.shippingAddress?.street || ""}</p>
            <p>${data.shippingAddress?.city || ""}, ${data.shippingAddress?.state || ""} ${
              data.shippingAddress?.pincode || ""
            }</p>
            <p>Phone: ${data.shippingAddress?.phone || data.customerPhone || ""}</p>
          </div>
          
          <div style="text-align: center; margin-top: 30px; color: #666; font-size: 14px;">
            <p>Thank you for shopping with SS Balaramapuram Handlooms!</p>
            <p>If you have any questions, please contact our customer service at support@ssbalaramapuram.com</p>
          </div>
        </div>
      `,
    },
    ...
    order_delivered: {
      ...
      body: (data) => `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
          ...
          <div style="margin-bottom: 20px;">
            <h2 style="color: #333;">Items Delivered</h2>
            <table style="width: 100%; border-collapse: collapse;">
              <thead>
                <tr style="background-color: #f3f4f6;">
                  <th style="padding: 10px; text-align: left; border-bottom: 1px solid #e0e0e0;">Product</th>
  ...
  private smsTemplates: Record<string, SmsTemplate> = {
    ...
    payment_success: {
      body: (data) =>
        `SS Balaramapuram: Payment of ₹${data.orderTotal} received for order #${data.orderNumber}. Thank you for your purchase!`,
    },
    order_shipped: {
      body: (data) =>
  ...
  async sendSms(type: string, data: NotificationData): Promise<boolean> {
    try {
      ...
      console.log(`Message: ${template.body(data)}`)
  ...
  async testNotification(type: string, email?: string): Promise<boolean> {
    try {
      ...
      return await this.sendNotification(type, testData, ["email"])
    } catch (error) {
